...
def refill_database(sessionmaker_):
    with sessionmaker_() as s:
        ...
        s.execute(stmt)
        stmt = text("""insert into posts (subscription_id, content, rating) values (1, 'Товарищеские матчи. «МЮ» против «Арсенала», «Челси» играет с «Брайтоном», игра «Барсы» и «Юве» отменена', 1), (1, 'Мбаппе интересен «Тоттенхэму», «МЮ» и «Челси». Крупнейшее предложение сделает «Аль-Хилаль», но «ПСЖ» знает, что Килиан к саудовцам не поедет', 2), (1, ' Хусаинов об удалении у «Пари НН»: «Александров помешал возникновению момента – прямая красная. Для такого решения нужны яйца»', 3), (1, '«Рома» не взяла Шомуродова в турне по Португалии. Форвард не входит в планы Моуринью', 4), (1, 'Глава «Локо» о видео Дзюбы с торчащим игрушечным бананом: «Я не видел. Если вы это смотрели, значит, вам интересно»', 5), (1, 'Джузеппе Росси завершил карьеру. Экс-форварду «МЮ», «Вильярреала» и сборной Италии 36 лет', 6), (1, 'Гендиректор «Рубина»: «Задача – вернуться в топ России. О борьбе только за сохранение места речи нет»', 7), (1, 'Орлов об удалении у «Пари НН»: «Бакаев выходил один на один, фол тянул на красную. Все по закону»', 8), (1, 'Глава «Локо» о Fan ID: «Продали около тысячи абонементов на сезон. У всех московских клубов меньше 10 000. Не та ситуация, которой мы могли бы гордиться»', 9), (1, 'Гильермо Абаскаль: «Уважаем философию «Спартака» – хотим доминировать, атаковать. Если против тебя стоят стеной, нужно знать, как это преодолеть»', 10);""")
        s.execute(stmt)
        stmt = text("""insert into posts (subscription_id, content, rating) values (2, 'Россиянам объяснили правила начисления отпускных', 5), (2, 'В полях под Тулой нашли ловушки из заточенной арматуры', 6), (2, 'В российском регионе зарегистрировали случаи лихорадки Западного Нила', 5), (2, 'В Бородино прошел финал игры «Зарница 2.0»', 4), (2, 'Минтранс анонсировал новые меры регулирования движения электросамокатов', 5);""")
        s.execute(stmt)
        stmt = text("""insert into posts (subscription_id, content, rating) values (3, 'В Турции произошло землетрясение', 4), (3, 'Синоптики спрогнозировали переход аномальной жары из Европы в Турци', 5), (3, 'Тещу президента Южной Кореи задержали за подделку документа', 6), (3, 'Трамп пожаловался на невиданное ранее давление на своих юристов', 5), (3, 'Гонконг ограничил движение общественного транспорта и занятия в школах', 3);""")
        s.execute(stmt)
        stmt = text("""insert into posts (subscription_id, content, rating) values (4, 'Глава Парагвая отметил столетие ВВС прыжком с парашютом', 1), (4, 'США не смогли повлиять на выборы в Парагвае, считает посол Писарев', 1)""")
        s.execute(stmt)
        stmt = text("""insert into posts (subscription_id, content, rating) values (5, 'МВД и «Стопнаркотик» договорились выявлять незаконные крипто-транзакции', 8), (5, 'Рынку криптовалют предсказали ужасный сценарий', 7), (5, 'Обнаружена новая опасность криптовалюты', 8)""")
        s.execute(stmt)
